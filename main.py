...
DATA_FILE = os.getenv("DATA_FILE", "metallic_ai_data.json")
...
class HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Metallic AI System Healthy")
...
def fetch_live_model_catalog() -> List[str]:
    ...
    headers = {"Authorization": f"Bearer {AI_API_KEY}", "User-Agent": "MetallicAIBot/1.0"}
    try:
...
SYSTEM_PROMPT = (
    "Your name and identity is strictly Metallic AI. "
    "Never identify yourself by the name of any underlying model, lab, or company that powers you. "
    "If asked who made you or what model you are, say only that you are Metallic AI. "
...
def scrub_identity_leaks(text: str) -> str:
    ...
    return cleaned.strip() or "I am Metallic AI, ready to help."
...
# -------------------------------------------------------------
@bot.tree.command(name="chat", description="Chat with Metallic AI")
...
async def ping_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"⚡ Metallic AI Gateway: `{ping_ms}ms` | Active Engine: `{ACTIVE_ENGINE}`",
        ephemeral=True
    )
...
async def stats_cmd(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="📊 Metallic AI Stats", color=discord.Color.blue())
...
async def dashboard_cmd(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🧭 Metallic AI — Operations Dashboard", color=discord.Color.gold())
...
async def broadcast_cmd(interaction: discord.Interaction, message: str):
    ...
    for guild in bot.guilds:
        ...
        if target:
            try:
                await target.send(f"📢 **Announcement from the Metallic AI team:**\n{message}")
                sent += 1
            except Exception:
                failed += 1
        else:
...
async def help_cmd(interaction: discord.Interaction):
    embed = discord.Embed(title="Metallic AI — Command Directory", color=discord.Color.blurple())
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(name="Chat with Metallic AI"))
